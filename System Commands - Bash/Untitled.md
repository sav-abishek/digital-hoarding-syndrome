Notes by Cherian GeorgeNotes by Cherian George  
https://github.com/cheriangeorge/System-Commands-coursehttps://github.com/cheriangeorge/System-Commands-course  
Week 1 NotesWeek 1 Notes  
Terminal EmulatorsTerminal Emulators  
Terminal  
Konsole  
xterm  
guake  
...
username@hostname:~$  
~$ is the path  
Commands and FlagsCommands and Flags  
uname  
...
ls - a : all . displays hidden files - l : use a long listing format - i : print index number of each file (inode) - s : shows blocks occupied by each file -  
1 : each file name on a separate line  
output of ls -l : drwxr-xr-x 5 ckg ckg 12288 Nov 25 10:00 Documents ( d is file type ; rwxr-xr-x owner,group,others permissions ; 5 no  
of hard links ; ckg is owner ; ckg is group ; last modified time stamp ; filename)  
...
s Socket file  
p named pipe  
Viewing and Adding to filesViewing and Adding to files  
...
Hard links and Soft linksHard links and Soft links  
...
If you want to access a file, all its parent direcories should have xx permission. This works even without r and w permissions if you know the path.
...
Command Line EnvironmentsCommand Line Environments  
Cloud - replit and cocalc  
Phone - Termux by Fredrick Fornwall  
File System of Linux OSFile System of Linux OS  
...
/dev device files (different character in long format of file listing 'c' instead of 'l' or 'd'. 'c' indicates character file - means you can read from it character by  
character. if first character is 'b' they are block devices typically hdds - the block devices are made available as files.)  
...
ShareableShareable UnsharableUnsharable  
static /usr and /opt /etc and /boot  
variable /var/mail /var/run and  
/var/lock  
